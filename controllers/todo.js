...
exports.getTodo = async (req,res,next)=>{
   ...
   try
   {
      ...
      //  res.status(200).json({data});
        let count = await Todo.count({where:{userId: req.user.id}})
        totalItems = count;

        let data = await req.user.getTodos({offset: (page-1)*Items_Per_Page, limit: Items_Per_Page})
        ///console.log(data);
        res.status(200).json({
            data,
            info: {
...
exports.addTodo = async (req,res,next)=>{
    ...
    try{
        ...
        if(!tname || !tdescription || !category){
...
exports.deleteTodo = async (req,res,next)=>{
    try{
        ...
        //res.sendStatus(200);
        await req.user.getTodos({where:{id:userId}}).then(todo=>{
            let findTodo = todo[0];
            findTodo.destroy();
            res.sendStatus(200);
        })
